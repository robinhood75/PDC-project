...
def decode_utf8_8bits_adapted(H_hat, N0_4, N0_3):

    # Might be useless if N0_4 and N0_3 are differently used.
    if N0_4 % 3 != 0 or N0_3 % 3 != 0:
        raise ValueError('Warning: N0_i values should be multiple of 3')
    
    N0_4_eff = 2*N0_4//3
    N0_3_eff = 2*N0_3//3
    N0_eff = N0_4_eff + N0_3_eff

    out, n = load_signal(N0_4 + N0_3)

    out_clean = [out[i] for i in range(len(out)) if i % 3 != H_hat]
    if len(out_clean) % N0_eff != 0:
        raise ValueError('Warning: length of out_clean should be multiple of N0_eff')
    
    nbWords = n // (N0_4*16 + N0_3*8)
    wordLen = N0_4_eff*16 + N0_3_eff*8
    assert len(out_clean) == nbWords*wordLen
    print(f"Found {nbWords} words!")

    words_duplicated = [out_clean[i*wordLen:(i+1)*wordLen] for i in range(nbWords)]

    def average(x, nbDuplicates):
        nbSingleElement = len(x)//nbDuplicates
        return np.array([np.sign(np.array(sum(x[i*nbDuplicates:(i+1)*nbDuplicates]))) for i in range(nbSingleElement)]).astype(int)

    words_splitted = [(average(x[:16*N0_4_eff], N0_4_eff), average(x[-8*N0_3_eff:], N0_3_eff)) for x in words_duplicated]
    for w in words_splitted:
        s4 = sum(w[0])
        s3 = sum(w[1])
        print(w, '\t-> Error' if s4 != -15 or s3 != -7 else '')

index, H_MAP = estimate_H()
print("Deleted index: {} (from the following estimate: {})\n".format(index, H_MAP))

decoded_string = decode_utf8_8bits_adapted(index, N0, N0)
